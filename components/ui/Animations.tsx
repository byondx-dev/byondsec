  ...
  return (
    <>
      ...
      <div
        ...
        className={`absolute flex size-full transform-gpu animate-orbit items-center justify-center rounded-full border-none bg-transparent [animation-delay:calc(var(--delay)*1000ms)] ${
...
export const Terminal: React.FC<TerminalProps> = ({ className }) => {
    ...
    useEffect(() => {
        const sequence = [
            ...
            { text: "user@byondSEC:~# _", delay: 5800 }
        ];
    ...
    return (
        <div className={`font-mono text-xs md:text-sm bg-[#050505] border border-white/10 rounded-lg overflow-hidden shadow-2xl ${className}`}>
            ...
            <div className="p-4 h-[250px] md:h-[300px] overflow-hidden flex flex-col justify-end">
                {lines.map((line, i) => (
                    <div key={i} className={`${line.includes("SUCCESS") || line.includes("session") ? 'text-primary' : 'text-gray-300'}`}>
                        {line}
                    </div>
                ))}
            </div>
